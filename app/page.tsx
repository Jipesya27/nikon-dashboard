...
export default function NikonDashboard() {
  ...
  const handleForgotPwSubmit = async (e: React.FormEvent) => {
    ...
    if (data) {
      if (data.role === 'Admin') {
        setForgotPwMessage('Akun Admin hanya dapat di-reset melalui database Supabase secara langsung demi keamanan.');
      } else {
        setForgotPwMessage('Silakan hubungi Administrator (Admin) untuk mereset password akun Anda.');
      }
    } else {
  ...
  const openModal = (action: 'create'|'edit'|'reset_pw', type: 'claim'|'warranty'|'promo'|'service'|'budget'|'karyawan', item?: any) => {
    ...
    if (type === 'claim') { 
       ...
       if (action === 'reset_pw') { 
           setKaryawanForm({ id_karyawan: item.id_karyawan, username: item.username, password: '' }); 
       } else { 
                 ...
                 <form id="budgetForm" onSubmit={handleSaveBudget} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                       ...
                       <div>
                          ...
                          <input required type="text" value={budgetForm.budget_source || ''} onChange={e => setBudgetForm({...budgetForm, budget_source: e.target.value})} className="w-full border border-slate-300 bg-white text-slate-900 rounded-md px-3 py-2 text-sm" />
                       </div>
                    </div>
                    <div>
