...
// Tipe untuk PDF Budget Approval
...
export default function NikonDashboard() {
  ...
  const exportToCSV = () => {
    ...
    const headers = Object.keys(dataToExport[0]).filter(key => key !== 'created_at' && key !== 'items'); 
    const csvRows = []; csvRows.push(headers.join(','));
    for (const row of dataToExport) {
      const values = headers.map(header => { const val = row[header] === null || row[header] === undefined ? '' : String(row[header]); return `"${val.replace(/"/g, '""')}"`; });
      ...
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          headers.forEach((header, j) => {
            ...
            if (val === "") val = null; if (val === 'true') val = true; if (val === 'false') val = false; obj[header] = val;
          });
  ...
  const closeModal = () => { setIsModalOpen(false); setClaimForm({}); setWarrantyForm({}); setPromoForm({}); setServiceForm({}); setBudgetForm({items:[]}); setEditingId(null); };
  ...
  const getNamaPromo = (tipeBarang: string) => { if (!tipeBarang) return '-'; const matchedPromo = promos.find(p => tipeBarang.toLowerCase().includes(p.tipe_produk.toLowerCase()) || p.tipe_produk.toLowerCase().includes(tipeBarang.toLowerCase())); return matchedPromo ? matchedPromo.nama_promo : '-'; };
                 ...
                 <div className="overflow-y-auto flex-1 divide-y">
                   {filteredContacts.map((c: any) => (
                     <div key={c.nomor_wa} onClick={() => setSelectedWa(c.nomor_wa)} className={`p-4 cursor-pointer hover:bg-slate-50 transition ${selectedWa === c.nomor_wa ? 'border-l-4 border-blue-500 bg-blue-50/50' : 'border-l-4 border-transparent'}`}>
                       <div className="flex justify-between text-sm"><span className="font-bold truncate flex items-center gap-2">{getRealProfileName(c.nomor_wa)}{c.bicara_dengan_cs && <span className="bg-red-500 text-white text-[9px] px-1.5 py-0.5 rounded-full shadow-sm animate-pulse">🚨 Perlu CS</span>}</span><span className="text-[10px] text-slate-400 whitespace-nowrap">{new Date(c.waktu_pesan).toLocaleDateString('id-ID')}</span></div>
              ...
              <div className="bg-white rounded-lg shadow-sm border lg:col-span-2 flex flex-col h-full overflow-hidden relative">
                 {selectedWa ? (
                    <>
                      ...
                      <div className="flex-1 p-4 overflow-y-auto space-y-4" style={{ backgroundColor: '#efeae2', backgroundImage: `url('https://i.pinimg.com/originals/8c/98/99/8c98994518b575bfd8c949e91d20548b.jpg')`, backgroundSize: '400px', backgroundRepeat: 'repeat' }}>
                      ...
                      <form onSubmit={handleSendReply} className="p-4 border-t flex gap-2 bg-slate-50 z-10"><input type="text" value={replyText} onChange={e => setReplyText(e.target.value)} placeholder="Ketik pesan..." className="flex-1 border border-slate-300 rounded-full px-5 py-2.5 text-sm outline-none focus:border-blue-500 shadow-sm" /><button type="submit" disabled={!replyText.trim()} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-full text-sm font-medium disabled:opacity-50 transition shadow-sm">Kirim</button></form>
                    </>
        ...
        {activeTab === 'claims' && (
          <div className="bg-white rounded-lg shadow-sm border overflow-x-auto animate-fade-in">
             ...
             <tbody className="divide-y">{claims.map(c => (
               <tr key={c.id_claim} className="whitespace-nowrap hover:bg-slate-50"><td className="px-4 py-3 text-slate-700">{consumers[c.nomor_wa]||c.nomor_wa}</td><td className="px-4 py-3 font-mono">{c.nomor_seri}</td><td className="px-4 py-3">{c.tipe_barang}</td><td className="px-4 py-3 font-medium text-blue-700">{getNamaPromo(c.tipe_barang)}</td><td className="px-4 py-3">{c.tanggal_pembelian}</td><td className="px-4 py-3">{c.nama_toko || '-'}</td><td className="px-4 py-3 text-blue-500 font-medium">{c.link_nota_pembelian && <a href={c.link_nota_pembelian} target="_blank" rel="noreferrer" className="mr-3 hover:underline">Nota</a>} {c.link_kartu_garansi && <a href={c.link_kartu_garansi} target="_blank" rel="noreferrer" className="hover:underline">Garansi</a>}</td><td className="px-4 py-3 text-xs">{c.validasi_by_mkt} / {c.validasi_by_fa}</td><td className="px-4 py-3 flex gap-2"><button onClick={()=>openModal('edit','claim',c)} className="text-blue-600 text-xs font-medium hover:underline">Edit</button><button onClick={()=>handleDelete('claim',c.id_claim!)} className="text-red-600 text-xs font-medium hover:underline">Hapus</button></td></tr>
             ))}</tbody></table>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-50 p-4">
          <div className={`bg-white rounded-xl shadow-xl w-full ${activeTab === 'budgets' ? 'max-w-4xl' : 'max-w-2xl'} overflow-hidden flex flex-col max-h-[90vh]`}>
            ...
            <div className="p-6 overflow-y-auto">
               {/* Forms */}
               {activeTab === 'claims' && (<form id="claimForm" onSubmit={handleSaveClaim} className="space-y-4"><div><label className="block text-sm font-medium mb-1">Nomor WA</label><input required type="text" value={claimForm.nomor_wa || ''} onChange={e => setClaimForm({...claimForm, nomor_wa: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div><div><label className="block text-sm font-medium mb-1">Nomor Seri</label><input required type="text" value={claimForm.nomor_seri || ''} onChange={e => setClaimForm({...claimForm, nomor_seri: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium mb-1">Tipe Barang</label><input type="text" value={claimForm.tipe_barang || ''} onChange={e => setClaimForm({...claimForm, tipe_barang: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div><div><label className="block text-sm font-medium mb-1">Tgl Pembelian</label><input type="date" value={claimForm.tanggal_pembelian || ''} onChange={e => setClaimForm({...claimForm, tanggal_pembelian: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium mb-1">Validasi MKT</label><select value={claimForm.validasi_by_mkt || ''} onChange={e => setClaimForm({...claimForm, validasi_by_mkt: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm"><option value="Dalam Proses Verifikasi">Dalam Proses Verifikasi</option><option value="Valid">Valid</option><option value="Tidak Valid">Tidak Valid</option></select></div><div><label className="block text-sm font-medium mb-1">Validasi FA</label><select value={claimForm.validasi_by_fa || ''} onChange={e => setClaimForm({...claimForm, validasi_by_fa: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm"><option value="Dalam Proses Verifikasi">Dalam Proses Verifikasi</option><option value="Valid">Valid</option><option value="Tidak Valid">Tidak Valid</option></select></div></div><div className="grid grid-cols-2 gap-4 p-3 bg-blue-50 border border-blue-100 rounded-md"><div className="col-span-2"><p className="text-xs text-blue-700 font-medium mb-2">*Catatan: Notifikasi pengiriman WA HANYA aktif jika Anda mengisi Nomor Resi di bawah ini.</p></div><div><label className="block text-sm font-medium mb-1">Jasa Pengiriman</label><input type="text" value={claimForm.nama_jasa_pengiriman || ''} onChange={e => setClaimForm({...claimForm, nama_jasa_pengiriman: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" placeholder="JNE / J&T / dll" /></div><div><label className="block text-sm font-medium mb-1">Nomor Resi</label><input type="text" value={claimForm.nomor_resi || ''} onChange={e => setClaimForm({...claimForm, nomor_resi: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" placeholder="Masukkan nomor resi..." /></div></div></form>)}
               {activeTab === 'warranties' && (<form id="warrantyForm" onSubmit={handleSaveWarranty} className="space-y-4"><div><label className="block text-sm font-medium mb-1">Nomor Seri</label><input required type="text" value={warrantyForm.nomor_seri || ''} onChange={e => setWarrantyForm({...warrantyForm, nomor_seri: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div><div><label className="block text-sm font-medium mb-1">Tipe Barang</label><input required type="text" value={warrantyForm.tipe_barang || ''} onChange={e => setWarrantyForm({...warrantyForm, tipe_barang: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div><div className="grid grid-cols-3 gap-4"><div><label className="block text-sm font-medium mb-1">Status Validasi</label><select value={warrantyForm.status_validasi || ''} onChange={e => setWarrantyForm({...warrantyForm, status_validasi: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm"><option value="Menunggu">Menunggu</option><option value="Valid">Valid</option><option value="Tidak Valid">Tidak Valid</option></select></div><div><label className="block text-sm font-medium mb-1">Jenis Garansi</label><select value={warrantyForm.jenis_garansi || ''} onChange={e => setWarrantyForm({...warrantyForm, jenis_garansi: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm"><option value="Jasa 30%">Jasa 30%</option><option value="Extended to 2 Year">Extended to 2 Year</option></select></div><div><label className="block text-sm font-medium mb-1">Lama Garansi</label><select value={warrantyForm.lama_garansi || ''} onChange={e => setWarrantyForm({...warrantyForm, lama_garansi: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm"><option value="1 Tahun">1 Tahun</option><option value="2 Tahun">2 Tahun</option><option value="Tidak Garansi">Tidak Garansi</option></select></div></div></form>)}
               {activeTab === 'promos' && (<form id="promoForm" onSubmit={handleSavePromo} className="space-y-4"><div><label className="block text-sm font-medium mb-1">Nama Promo</label><input required type="text" value={promoForm.nama_promo || ''} onChange={e => setPromoForm({...promoForm, nama_promo: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div><div><label className="block text-sm font-medium mb-1">Tipe Produk</label><input required type="text" value={promoForm.tipe_produk || ''} onChange={e => setPromoForm({...promoForm, tipe_produk: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium mb-1">Tgl Mulai</label><input required type="date" value={promoForm.tanggal_mulai || ''} onChange={e => setPromoForm({...promoForm, tanggal_mulai: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div><div><label className="block text-sm font-medium mb-1">Tgl Selesai</label><input required type="date" value={promoForm.tanggal_selesai || ''} onChange={e => setPromoForm({...promoForm, tanggal_selesai: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div></div></form>)}
               {activeTab === 'services' && (<form id="serviceForm" onSubmit={handleSaveService} className="space-y-4"><div><label className="block text-sm font-medium mb-1">Nomor Tanda Terima</label><input required type="text" value={serviceForm.nomor_tanda_terima || ''} onChange={e => setServiceForm({...serviceForm, nomor_tanda_terima: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div><div><label className="block text-sm font-medium mb-1">Nomor Seri</label><input required type="text" value={serviceForm.nomor_seri || ''} onChange={e => setServiceForm({...serviceForm, nomor_seri: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div><div><label className="block text-sm font-medium mb-1">Status Service</label><input required type="text" value={serviceForm.status_service || ''} onChange={e => setServiceForm({...serviceForm, status_service: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div></form>)}

               {activeTab === 'budgets' && (
                 <form id="budgetForm" onSubmit={handleSaveBudget} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4"><div><label className="block text-sm font-medium mb-1">Proposal No</label><input required type="text" value={budgetForm.proposal_no || ''} onChange={e => setBudgetForm({...budgetForm, proposal_no: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div><div><label className="block text-sm font-medium mb-1">Title</label><input required type="text" value={budgetForm.title || ''} onChange={e => setBudgetForm({...budgetForm, title: e.target.value})} className="w-full border rounded-md px-3 py-2 text-sm" /></div></div>
                    ...
                    <div className="mt-6 border-t pt-4">
                       <div className="flex justify-between items-center mb-2"><label className="block text-sm font-medium">Rincian Budget (Items)</label><button type="button" onClick={() => setBudgetForm({...budgetForm, items: [...(budgetForm.items || []), {purpose: '', qty: 1, cost_unit: 0, value: 0}]})} className="bg-slate-200 px-3 py-1 rounded text-xs font-medium">+ Tambah Item</button></div>
                       {budgetForm.items?.map((item, index) => (
                         <div key={index} className="flex gap-2 mb-2 items-end">
                            <div className="flex-1"><label className="text-xs text-slate-500">Purpose</label><input type="text" value={item.purpose} onChange={e => { const newItems = [...(budgetForm.items||[])]; newItems[index].purpose = e.target.value; setBudgetForm({...budgetForm, items: newItems})}} className="w-full border rounded px-2 py-1 text-sm"/></div>
                            <div className="w-16"><label className="text-xs text-slate-500">Qty</label><input type="number" value={item.qty} onChange={e => { const newItems = [...(budgetForm.items||[])]; newItems[index].qty = Number(e.target.value); newItems[index].value = newItems[index].qty * newItems[index].cost_unit; setBudgetForm({...budgetForm, items: newItems})}} className="w-full border rounded px-2 py-1 text-sm"/></div>
                            <div className="w-32"><label className="text-xs text-slate-500">Cost/Unit</label><input type="number" value={item.cost_unit} onChange={e => { const newItems = [...(budgetForm.items||[])]; newItems[index].cost_unit = Number(e.target.value); newItems[index].value = newItems[index].qty * newItems[index].cost_unit; setBudgetForm({...budgetForm, items: newItems})}} className="w-full border rounded px-2 py-1 text-sm"/></div>
                            ...
                            <button type="button" onClick={() => { const newItems = [...(budgetForm.items||[])]; newItems.splice(index, 1); setBudgetForm({...budgetForm, items: newItems}); }} className="bg-red-100 text-red-600 px-2 py-1 rounded text-sm mb-0.5">X</button>
                         </div>
                       ))}
                       <div className="flex justify-end items-center mt-4 gap-4">
                          <button type="button" onClick={() => { const total = (budgetForm.items || []).reduce((acc, curr) => acc + curr.value, 0); setBudgetForm({...budgetForm, total_cost: total}); }} className="text-xs text-blue-600 underline">Hitung Ulang Total</button>
            ...
            <div className="px-6 py-4 border-t bg-slate-50 flex justify-end gap-3"><button onClick={closeModal} className="px-4 py-2 border border-slate-300 bg-white hover:bg-slate-50 rounded-md text-sm font-medium transition">Batal</button><button type="submit" form={activeTab === 'claims' ? 'claimForm' : activeTab === 'warranties' ? 'warrantyForm' : activeTab === 'services' ? 'serviceForm' : 'budgetForm'} disabled={isSubmitting} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-sm font-medium transition disabled:opacity-50">{isSubmitting ? 'Menyimpan...' : 'Simpan Data'}</button></div>
          </div>
        </div>
      )}
      ...
      {printData && (
        <div className="hidden print:block absolute top-0 left-0 w-full bg-white text-black font-sans z-[100] min-h-screen pb-10" style={{ fontSize: '13px' }}>
          ...
          <div className="flex justify-between items-start border-b-2 border-black pb-2 mb-4">
             <div className="flex items-center gap-4">
               {/* Jika Anda memiliki logo perusahaan, Anda bisa meletakkannya di sini dengan tag img */}
          ...
          <div className="flex gap-2 mb-4 text-center">
             ...
             <div className="border-2 border-black w-48 flex flex-col">
                <div className="border-b-2 border-black p-1 font-bold">Proposed/Prepared by</div>
                <div className="flex-1 flex items-end justify-center pb-2 font-bold">{printData.drafter_name || 'Firza'}</div>
                <div className="border-t border-black flex text-xs divide-x divide-black">
             ...
             <div className="border-2 border-black flex-1 flex flex-col relative pt-5">
                ...
                <div className="border-b border-black p-1 font-bold">Management Approval</div>
             ...
             {/* Box 3: Finance */}
             <div className="border-2 border-black w-48 flex flex-col">
                <div className="border-b-2 border-black p-1 font-bold">Finance Accounting Dept</div>
          ...
          <div className="border-2 border-black mb-4">
             <div className="flex border-b border-black"><div className="w-40 font-bold p-1.5 border-r border-black">Title:</div><div className="p-1.5 flex-1 font-bold">{printData.title}</div></div>
             <div className="flex border-b border-black"><div className="w-40 font-bold p-1.5 border-r border-black">Proposal No:</div><div className="p-1.5 flex-1">{printData.proposal_no}</div></div>
             <div className="flex border-b border-black"><div className="w-40 font-bold p-1.5 border-r border-black">Period:</div><div className="p-1.5 flex-1">{printData.period}</div></div>
             <div className="flex border-b border-black"><div className="w-40 font-bold p-1.5 border-r border-black">Objectives:</div><div className="p-1.5 flex-1 whitespace-pre-wrap">{printData.objectives}</div></div>
             <div className="flex border-b border-black"><div className="w-40 font-bold p-1.5 border-r border-black">Detail of Activity:</div><div className="p-1.5 flex-1 whitespace-pre-wrap min-h-[30px]">{printData.detail_activity}</div></div>
             <div className="flex border-b border-black"><div className="w-40 font-bold p-1.5 border-r border-black">Expected Result:</div><div className="p-1.5 flex-1 whitespace-pre-wrap">{printData.expected_result}</div></div>
             <div className="flex"><div className="w-40 font-bold p-1.5 border-r border-black">Total Cost:</div><div className="p-1.5 flex-1 font-bold">Rp {Number(printData.total_cost).toLocaleString('id-ID')}</div></div>
          </div>
             ...
             <table className="w-full border-collapse border-2 border-black">
                ...
                <tbody>
                   ...
                   <tr>
                      ...
                      <td colSpan={2} className="border border-black p-1 text-right font-bold pr-4">Subtotal</td>
                      <td className="border border-black p-1 text-right font-bold">{Number(printData.total_cost).toLocaleString('id-ID')}</td>
                   </tr>
                   <tr>
                      <td colSpan={3} className="border-l border-b border-black bg-white"></td>
                      <td colSpan={2} className="border border-black p-1 text-right font-bold pr-4">Total</td>
                      <td className="border border-black p-1 text-right font-bold">{Number(printData.total_cost).toLocaleString('id-ID')}</td>
                   </tr>
                </tbody>
             </table>
