...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!ALLOWED_MIME.includes(file.type)) return NextResponse.json({ error: 'Tipe file tidak didukung. Gunakan JPG, PNG, atau WEBP.' }, { status: 400 });
    if (file.size > MAX_SIZE) return NextResponse.json({ error: 'Ukuran file maksimal 10 MB' }, { status: 400 });

    const token = await getAccessToken();
    const folderId = await getOrCreateFolder('AltaSolution', ROOT_FOLDER_ID, token);
    const ext = file.name.split('.').pop() || 'jpg';
    const baseName = sanitizeFileName(file.name.replace(/\.[^.]+$/, ''));
    const fileName = `Produk_${baseName}_${Date.now()}.${ext}`;
