            ...
            <div className="flex flex-1 overflow-hidden relative">
               ...
               <div className={`fixed md:relative z-20 md:z-auto top-0 left-0 h-full w-56 bg-white border-r border-gray-200 flex flex-col overflow-hidden transition-transform duration-200 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
                  ...
                  <div className="flex items-center gap-2.5 px-4 py-3.5 border-b border-gray-100 shrink-0">
                     <div className="w-7 h-7 bg-[#FFE500] rounded-md flex items-center justify-center text-black font-bold text-xs shrink-0">N</div>
                     <span className="text-sm font-semibold text-gray-800 truncate">Nikon Dashboard</span>
                  </div>

                  {/* User info */}
                  <div className="flex items-center gap-2.5 px-4 py-2.5 border-b border-gray-100 shrink-0">
                     <div className="w-7 h-7 rounded-full bg-[#FFE500] flex items-center justify-center text-black font-semibold text-xs shrink-0">
                        {currentUser?.nama_karyawan?.substring(0, 2).toUpperCase() ?? '??'}
                     </div>
                     <div className="min-w-0">
                        <p className="text-xs font-semibold text-gray-800 truncate">{currentUser?.nama_karyawan}</p>
                  ...
                  <div className="flex-1 overflow-y-auto py-2">
                     {groupedVisibleTabs.map(group => (
                        <div key={group.category} className="mb-1">
                           <h3 className="px-3 pt-3 pb-1 text-[10px] font-semibold text-gray-400 uppercase tracking-wider">{group.category}</h3>
                              ...
                              {group.tabs.map(tab => {
                                 ...
                                 return extUrl ? (
                                    <a key={tab.id} href={extUrl} target="_blank" rel="noopener noreferrer" onClick={() => setSidebarOpen(false)}
                                       className="flex items-center gap-2 px-2.5 py-2 rounded-md text-xs font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-colors group">
                                       <span className="flex-1 truncate">{cleanLabel}</span>
                                       <svg className="w-3 h-3 shrink-0 opacity-40 group-hover:opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                                    </a>
                                 ) : (
                                    <button key={tab.id} onClick={() => { setActiveTab(tab.id); setSidebarOpen(false); }}
                                       className={`w-full flex items-center gap-2 px-2.5 py-2 rounded-md text-xs font-medium transition-colors ${activeTab === tab.id ? 'bg-[#FFE500] text-black' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-800'}`}>
                                       <span className="flex-1 text-left truncate">{cleanLabel}</span>
                                       {tab.count !== undefined && (
                                          <span className={`text-[10px] px-1.5 py-0.5 rounded font-semibold shrink-0 ${activeTab === tab.id ? 'bg-black/15 text-black' : 'bg-gray-100 text-gray-500'}`}>{tab.count}</span>
                                       )}
                                    </button>
                                 );
                              })}
                     ...
                     <div className="mt-1 pt-2 border-t border-gray-100 mx-2">
                        <h3 className="px-1 pt-2 pb-1 text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Halaman Lain</h3>
                        <div className="space-y-px">
                           {[
                              ...
                              <a key={link.href} href={link.href} target="_blank" rel="noopener noreferrer" onClick={() => setSidebarOpen(false)}
                                 className="flex items-center gap-2 px-2.5 py-2 rounded-md text-xs text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-colors group">
                                 <span className="flex-1 truncate">{link.label}</span>
                                 <svg className="w-3 h-3 shrink-0 opacity-0 group-hover:opacity-40 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                              </a>
                           ))}
                        </div>
                     </div>
                  ...
                  <div className="shrink-0 px-3 py-3 border-t border-gray-100">
                     <button
                        ...
                        className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-md text-xs font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 transition-colors disabled:opacity-40 border border-gray-200"
                     >
               ...
               {activeTab === 'dashboard' && (
                  <div className="animate-fade-in space-y-5">
                     ...
                     <div className="flex items-center justify-between">
                        <div>
                           <h2 className="text-lg font-semibold text-gray-900">Selamat datang, {currentUser?.nama_karyawan}</h2>
                           <p className="text-sm text-gray-400 mt-0.5">{new Date().toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', timeZone: 'Asia/Jakarta' })}</p>
                        </div>
                        <Image src="/nikon-logo.svg" alt="Nikon" width={80} height={32} className="h-8 w-auto opacity-60" />
                     </div>

                     {/* STATISTICS CARDS */}
                     <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                        {[
                           { label: 'Pesan', value: messagesCount, sub: 'Chat masuk', accent: '#3b82f6', onClick: () => setActiveTab('messages') },
                           { label: 'Claim Promo', value: claims.length, sub: `${claimStatusCounts.Pink ?? 0} tunggu resi`, accent: '#f472b6', onClick: () => setActiveTab('claims') },
                           { label: 'Garansi', value: warranties.length, sub: 'Terdaftar', accent: '#10b981', onClick: () => setActiveTab('warranties') },
                           { label: 'Konsumen', value: consumersList.length, sub: 'Terdata', accent: '#8b5cf6', onClick: () => setActiveTab('konsumen') },
                        ].map(s => (
                           <button key={s.label} onClick={s.onClick}
                              className="stat-card text-left hover:border-gray-300 transition-colors group"
                              style={{ borderTop: `3px solid ${s.accent}` }}>
                              <p className="stat-label">{s.label}</p>
                              <p className="stat-value mt-1">{s.value}</p>
                              <p className="text-xs text-gray-400 mt-1 group-hover:text-gray-600 transition-colors">{s.sub}</p>
                           </button>
                        ))}
                     </div>
                     ...
                     <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                        {[
                           { label: 'Service', value: services.length, sub: 'Unit masuk', accent: '#f59e0b', onClick: () => setActiveTab('services') },
                           { label: 'Peminjaman', value: lendingRecords.length, sub: `${lendingRecords.filter(l => l.status_peminjaman === 'aktif').length} aktif`, accent: '#6366f1', onClick: () => setActiveTab('lending') },
                           { label: 'Event', value: events.length, sub: `${eventRegistrations.length} peserta`, accent: '#ec4899', onClick: () => setActiveTab('events') },
                           { label: 'Karyawan', value: karyawans.length, sub: 'Anggota tim', accent: '#64748b', onClick: () => setActiveTab('userrole') },
                        ].map(s => (
                           <button key={s.label} onClick={s.onClick}
                              className="stat-card text-left hover:border-gray-300 transition-colors group"
                              style={{ borderTop: `3px solid ${s.accent}` }}>
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-white rounded-xl border border-gray-200 p-5">
                           ...
                           <div className="space-y-2">
                              {[
                                 { label: 'Buka tab Pesan', tab: 'messages' },
                                 { label: 'Lihat Claim Promo', tab: 'claims' },
                                 { label: 'Import Data', tab: 'import' },
                                 { label: 'Bot Settings', tab: 'botsettings' },
                              ].map(a => (
                                 <button key={a.tab} onClick={() => setActiveTab(a.tab)}
                                    className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg border border-gray-100 hover:border-[#FFE500] hover:bg-yellow-50 text-sm text-gray-700 font-medium transition-colors">
                                    {a.label}
                                    <svg className="w-4 h-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5l7 7-7 7"/></svg>
                                 </button>
                              ))}
                           </div>
                        </div>
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 p-5">
                           <div className="flex items-center justify-between mb-3">
                              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Status database</p>
                              <button
                                 onClick={async () => {
                                    setDbChecking(true);
                                    try {
                              ...
                              >{dbChecking ? 'Memeriksa...' : 'Cek koneksi'}</button>
                           </div>
                           {!dbCheckResult ? (
                              <p className="text-sm text-gray-400">Klik &ldquo;Cek koneksi&rdquo; untuk memeriksa status database.</p>
                           ) : (
                              <div className="text-xs space-y-1 font-mono">
                                 {dbCheckResult.error ? (
                                    ...
                                    <>
                                       <p className="text-green-700">✅ Terhubung</p>
                                       <p className={dbCheckResult.serviceKeySet ? 'text-green-700' : 'text-red-600'}>
                                          {dbCheckResult.serviceKeySet ? '✅' : '❌'} Service key: {dbCheckResult.serviceKeySet ? 'OK' : 'MISSING'}
                                       </p>
                                       {dbCheckResult.tables && typeof dbCheckResult.tables === 'object' && Object.entries(dbCheckResult.tables as Record<string, unknown>).map(([t, c]) => (
                                          <p key={t} className={String(c).startsWith('ERR') ? 'text-red-600' : 'text-gray-500'}>• {t}: {String(c)} baris</p>
                                       ))}
                                    </>
                                 )}
                              </div>
               ...
               {activeTab !== 'import' && activeTab !== 'lending' && activeTab !== 'messages' && activeTab !== 'eventreport' && (
                  <div className="flex flex-wrap gap-3 justify-between items-center mb-5 text-gray-900">
                     ...
                     <div className="flex flex-wrap gap-2 items-center">
                        {activeTab === 'claims' && (
                           <>
                              <button onClick={handleExportCSVClaim} className="flex items-center gap-1.5 bg-white border border-gray-200 text-gray-700 px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-gray-50 transition shadow-sm">Export CSV</button>
                              <button onClick={handleTandaTerimaCSV} className="flex items-center gap-1.5 bg-white border border-gray-200 text-gray-700 px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-gray-50 transition shadow-sm">Tanda Terima</button>
                              <button onClick={() => resiCsvInputRef.current?.click()} className="flex items-center gap-1.5 bg-white border border-gray-200 text-gray-700 px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-gray-50 transition shadow-sm">Upload Resi</button>
                              <input ref={resiCsvInputRef} type="file" accept=".csv" className="hidden" onChange={handleUploadResiCSV} />
                              <button onClick={() => openModal('create', 'claim')} className="bg-[#FFE500] hover:bg-[#E5CE00] text-black px-3 py-1.5 rounded-lg text-xs font-bold transition shadow-sm">+ Tambah Claim</button>
                           </>
                        )}
