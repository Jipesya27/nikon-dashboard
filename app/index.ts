...
export interface RiwayatPesan { id_pesan?: string; nomor_wa: string; nama_profil_wa: string; arah_pesan: 'IN' | 'OUT'; isi_pesan: string; waktu_pesan: string; bicara_dengan_cs?: boolean; created_at?: string; url_media?: string; jenis_pesan?: 'chat' | 'system' | 'bot'; }
...
export interface ExpenseClaimItem { tanggal: string; description: string; nominal: number; }
export interface ExpenseClaim { id?: string; created_at?: string; created_by: string; nama_pembuat: string; to_person: string; claim_date: string; status: 'draft' | 'submitted' | 'approved' | 'rejected'; catatan?: string; items: ExpenseClaimItem[]; receipt_urls: string[]; total_nominal: number; }
